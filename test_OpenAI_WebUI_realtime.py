...
def realtime_sdp_proxy():
    ...
    try:
        ...
        # ✅ 日本語指示を含む初期セッション設定を注入
        # --- gpt-realtime の transcript null 問題回避のため ---
        # whisper-1 を input_audio_transcription に明示指定し、
        # session初期更新として "input_audio_transcription" ブロックを付与
        if "a=openai-instructions" not in sdp_offer:
            sdp_offer = sdp_offer.replace(
                "m=audio",
                (
                    "a=openai-instructions: あなたは有能な日本語アシスタントです。すべての応答は日本語で返してください。\r\n"
                    "a=openai-defaults: input_audio_transcription.model=whisper-1\r\n"
                    "a=openai-defaults: input_audio_format=wav\r\n"
                    "a=openai-defaults: turn_detection.type=server_vad\r\n"
                    "a=openai-defaults: turn_detection.silence_duration_ms=2000\r\n"
                    "m=audio"
                )
            )

        modified_sdp = sdp_offer
        url = "https://api.openai.com/v1/realtime?model=gpt-realtime"
        res = requests.post(url, headers=headers, data=modified_sdp)
